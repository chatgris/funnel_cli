defmodule FunnelCli.Client do
  ...
  def register(host) do
    HTTPoison.post("#{host}/register", "")
      |> get_body
      |> process_response_body
  end

  def index(body, connection) do
    HTTPoison.post("#{connection["host"]}/index", body, headers_with_auth(connection))
      |> get_body
      |> process_response_body
  end

  def query(body, index_id, connection) do
    HTTPoison.post("#{connection["host"]}/index/#{index_id}", body, headers_with_auth(connection))
      |> get_body
      |> process_response_body
  end

  def queries(index_id, connection) do
    HTTPoison.get("#{connection["host"]}/index/#{index_id}/queries", headers_with_auth(connection))
      |> get_body
      |> process_response_body
  end

  def push(index_id, body, connection) do
    HTTPoison.post("#{connection["host"]}/index/#{index_id}/feeding", body, headers_with_auth(connection))
      |> get_body
  end
